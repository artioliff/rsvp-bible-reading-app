...
// Sample Bible data - structured for future API integration
...
export const getBookByName = (name: string): Book | undefined =>
  bibleBooks.find((b) => b.name === name);

export const getChapter = (bookName: string, chapterNum: number): Chapter | undefined => {
...
// Book list for selection UI
export const bookNames = bibleBooks.map((b) => b.name);
...
export const getVerseNumbers = (bookName: string, chapterNum: number): number[] => {
  const chapter = getChapter(bookName, chapterNum);
